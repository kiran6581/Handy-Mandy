...
def razorpayView(request):
    ...
    callback_url = request.build_absolute_uri('/category/paymenthandler/')    
...
def paymenthandler(request):
    ...
    if request.method == "POST":
        try:
            ...
            # Update cart items with the new order ID
            cartQuery = CartModel.objects.filter(userId=request.session['shippingUserId']) & CartModel.objects.filter(orderId=0)
            for i in cartQuery:
                cModel = CartModel(pk=i.pk)
                cModel.userId = i.userId
                cModel.vendorId = i.vendorId
            ...
            # Clear session data
            request.session.pop('shippingUserId', None)
            request.session.pop('shippingfname', None)
            request.session.pop('shippinglname', None)
            request.session.pop('shippingemail', None)
            request.session.pop('shippingcontact', None)
            request.session.pop('shippingaddress', None)
            request.session.pop('shippingcity', None)
            request.session.pop('shippingpincode', None)
            request.session.pop('shippingsubTotal', None)
            request.session.pop('shippinggstPercentage', None)
            request.session.pop('shippinggstAmount', None)
            request.session.pop('shippingdeliveryCharge', None)
            request.session.pop('shippinggrantTotal', None)
            request.session.pop('shippingpayment', None)
            request.session.pop('shippingpaymentVia', None)
            request.session.pop('shippingtransactionId', None)
            request.session.pop('shippingorderStatus', None)

            # Redirect to invoice page with the order ID
            return redirect('invoiceurl', orderId=oModel.pk)
        except Exception as e:
            print(f"Payment Error: {str(e)}")
            return redirect('indexurl')  # Redirect to home page on error
    else:
        return redirect('indexurl')  # Redirect to home page if not POST request
